...
for event in step_workflow.run(
    ...
    # Filter out RunOutputContentEvent from printing to reduce noise
    if not isinstance(
        event, (RunOutputContentEvent, ToolCallStartedEvent, ToolCallCompletedEvent)
    ):
        print(
...
for event in parallel_workflow.run(
    ...
    # Filter out RunOutputContentEvent from printing
    if not isinstance(event, RunOutputContentEvent):
        print(
            f"Event: {event.event if hasattr(event, 'event') else type(event).__name__}"
        )
