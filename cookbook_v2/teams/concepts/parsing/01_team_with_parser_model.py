"""
This example demonstrates how to use a parser model with teams for structured output.

The team uses a separate model for parsing structured responses, ensuring that
complex outputs are properly formatted according to the defined schema.
"""
...
class NationalParkAdventure(BaseModel):
    """Structured data model for national park adventure information."""
...
# Create team with parser model for structured output
national_park_expert = Team(
    model=OpenAIChat(id="gpt-4.1"),
    members=[itinerary_planner, weather_expert],
    response_model=NationalParkAdventure,
    parser_model=OpenAIChat(id="gpt-4o"),  # Separate model for parsing structured output
)

# List of national parks for random selection
national_parks = [
...
# Get structured response using parser model
selected_park = random.choice(national_parks)
run: RunResponse = national_park_expert.run(
    f"What is the best season to visit {selected_park}? Please provide a detailed one week itinerary for a visit to the park."
)

# Print the structured output
pprint(run.content)

# Alternative: Stream the response
