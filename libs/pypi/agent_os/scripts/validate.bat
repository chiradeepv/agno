...
SET "CURR_DIR=%~dp0"
SET "AGNO_OS_DIR=%CURR_DIR%\.."

ECHO.
ECHO ##################################################
ECHO # Validating agno_os
ECHO ##################################################
ECHO.
...
ECHO.
ECHO ##################################################
ECHO # Running: ruff check %AGNO_OS_DIR%
ECHO ##################################################
ECHO.

python -m ruff check "%AGNO_OS_DIR%"
IF %ERRORLEVEL% NEQ 0 (
...
ECHO.
ECHO ##################################################
ECHO # Running: mypy %AGNO_OS_DIR% --config-file %AGNO_OS_DIR%\pyproject.toml
ECHO ##################################################
ECHO.

python -m mypy "%AGNO_OS_DIR%" --config-file "%AGNO_OS_DIR%\pyproject.toml"
IF %ERRORLEVEL% NEQ 0 (
    ECHO [ERROR] mypy validation failed with exit code %ERRORLEVEL%
    EXIT /B %ERRORLEVEL%
)

ECHO [INFO] agno_os validation complete.
EXIT /B 0 