...
then
  ...
  UV_CUSTOM_COMPILE_COMMAND="./scripts/generate_requirements.sh upgrade" \
    uv pip compile ${AGNO_OS_DIR}/pyproject.toml --no-cache --upgrade -o ${AGNO_OS_DIR}/requirements.txt
else
  print_heading "Generating requirements.txt"
  UV_CUSTOM_COMPILE_COMMAND="./scripts/generate_requirements.sh" \
    uv pip compile ${AGNO_OS_DIR}/pyproject.toml --no-cache -o ${AGNO_OS_DIR}/requirements.txt
fi
