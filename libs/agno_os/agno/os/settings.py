...
@dataclass
class OSSettings:
    """Settings that can be used by any resource in the OS."""
    ...
    # OS git repo url
    os_repo: Optional[str] = None

    # default env for agno os commands
    default_env: Optional[str] = "dev"

    # default infra for agno os commands
    ...
    def _validate_and_set_keys(self):
        ...
        if self.test_key is None:
            if self.os_name is None:
                raise ValueError("`os_name` is None: Please set a valid value")
            if self.test_env is None:
                raise ValueError("`test_env` is None: Please set a valid value")
            self.test_key = f"{self.os_name}-{self.test_env}"
        ...
        if self.dev_key is None:
            ...
            self.dev_key = f"{self.os_name}-{self.dev_env}"
        ...
        if self.stg_key is None:
            ...
            self.stg_key = f"{self.os_name}-{self.stg_env}"
        ...
        if self.prd_key is None:
            ...
            self.prd_key = f"{self.os_name}-{self.prd_env}"
