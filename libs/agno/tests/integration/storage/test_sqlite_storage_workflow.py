...
def temp_db_path(tmp_path):
    ...
    db_file = tmp_path / "test_shared_db.db"
    yield str(db_file)
...
@pytest.fixture
def workflow_with_storage(shared_db):
    """Create a workflow with the test storage."""
    return SimpleWorkflow(storage=shared_db, name="TestWorkflow")
...
def test_workflow_session_storage(workflow_with_storage, shared_db):
...
def test_multiple_interactions(workflow_with_storage, shared_db):
...
def test_session_retrieval_by_user(shared_db):
    """Test retrieving sessions filtered by user ID."""
    # Create a session with a specific user ID
    workflow = SimpleWorkflow(storage=shared_db, user_id="test_user", name="UserTestWorkflow")
...
def test_session_deletion(workflow_with_storage, shared_db):
...
def test_get_all_session_ids(shared_db):
    ...
    workflow_1 = SimpleWorkflow(storage=shared_db, user_id="user1", workflow_id="workflow1", name="Workflow1")
    workflow_2 = SimpleWorkflow(storage=shared_db, user_id="user1", workflow_id="workflow2", name="Workflow2")
    workflow_3 = SimpleWorkflow(storage=shared_db, user_id="user2", workflow_id="workflow3", name="Workflow3")
    ...
    workflow1_sessions = shared_db.get_all_session_ids(entity_id="workflow1")
    assert len(workflow1_sessions) == 1

    # Filter by both
    filtered_sessions = shared_db.get_all_session_ids(user_id="user1", entity_id="workflow2")
    assert len(filtered_sessions) == 1


def test_drop_storage(shared_db):
    """Test dropping all sessions from storage."""
    # Create a few sessions
    for i in range(3):
        workflow = SimpleWorkflow(storage=shared_db, name=f"Workflow{i}")
...
def test_workflow_session_rename(workflow_with_storage, shared_db):
...
def test_workflow_rename(workflow_with_storage, shared_db):
